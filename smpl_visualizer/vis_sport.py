    ...
    def __init__(self, pl, sport='tennis', color='red', blur=False, num_exposure=30, real_shadow=False):
        ...
        if sport == 'tennis':
            ...
            if not self.real_shadow:
                ...
                self.shadow_actor = self.pl.add_mesh(shadow_mesh, color='#101010', 
                ...
                if self.blur: 
                    ...
                    for i in range(self.num_exposure):
                        shadow_mesh = pyvista.Circle(radius=0.05)
                        self.shadow_actors += [self.pl.add_mesh(shadow_mesh, color='#101010', 
                            diffuse=1, smooth_shading=True)]
        else:
            NotImplemented
...
class SportVisualizer(PyvistaVisualizer):
    ...
    def init_scene(self, init_args):
        ...
        if self.show_ball_target:
            self.ball_tar_actors = [TargetBounceActor(self.pl) for _ in range(self.num_actors)] 
        ...
        if self.show_stats:
            ...
            self.text_actor_pose = self.pl.add_text('', position=(30, 960), color='black', font_size=12)
    ...
    def update_scene_online(self, joint_pos=None, smpl_verts=None, racket_params=None, ball_params=None, ball_targets=None,
